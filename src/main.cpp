...
void ledOn();
char buf[100];
unsigned long last_msg_from_server_at = 0;
...
void setup()
{
  ...
  last_msg_from_server_at = millis();
...
void loop()
{
  ...
  if (millis() - last_msg_from_server_at > RESET_IF_NO_MSG_FROM_SERVER_MS)
  {
    snprintf(buf, sizeof(buf), "Not received any msg from server, resetting now");
    Log.log(buf);
    NVIC_SystemReset();
  }
}
